...
def load_dataset():
    T1_ori = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\Farm1.mat')
    T2_ori = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\Farm2.mat')
    mat_gt = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\GTChina1.mat')
    TT1 = T1_ori['imgh']
    TT2 = T2_ori['imghl']
    gt_hsi = mat_gt['label']
    TOTAL_SIZE = 63000
    ...
    # data_PCA_1 = pca.fit_transform(TT1.reshape(TT1.shape[0] * TT1.shape[1], -1))
    # TT1 = data_PCA_1.reshape(TT1.shape[0], TT1.shape[1], -1)
    # data_PCA_2 = pca.fit_transform(TT2.reshape(TT2.shape[0] * TT2.shape[1], -1))
    # TT2 = data_PCA_2.reshape(TT2.shape[0], TT2.shape[1], -1)


    # T1_ori = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\River_before.mat')
    # T2_ori = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\River_after.mat')
    # mat_gt = sio.loadmat('F:\第五个工作\CSANet\CSANet-main\datasets\Rivergt.mat')
...
def generate_iter(TRAIN_SIZE, train_indices,  TOTAL_SIZE, total_indices, VAL_SIZE,
    ...
    return train_iter,  valiada_iter, all_iter, x1_tensor_train,x2_tensor_train,y_tensor_train  # , y_test
